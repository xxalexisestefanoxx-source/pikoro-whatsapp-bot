...
export function participantLabel(participant, fallback = '') {
  const phone = participant?.phoneNumber || participant?.phone || (participant?.id?.endsWith('@s.whatsapp.net') ? participant.id : null)
  return `@${String(phone || participant?.id || fallback).split('@')[0]}`
}
...
async function updateParticipants(context, action, success) {
  ...
  await reply(sock, chat, message, `${success}: ${targetLabel(jids, metadata.participants || [])}`, jids)
}
...
export const commands = [
  ...
  { name: 'unban', async execute(context) { context.message._command = 'unban'; const { participants } = await getGroupContext(context); const jids = targetJids(context); context.store.chats[context.chat] ||= {}; context.store.chats[context.chat].bannedUsers = (context.store.chats[context.chat].bannedUsers || []).filter((jid) => !jids.includes(jid)); await reply(context.sock, context.chat, context.message, jids.length ? `✅ Baneo retirado: ${targetLabel(jids, participants)}` : 'Responde o etiqueta al usuario.') } },
  ...
  { name: 'mute', async execute(context) { context.message._command = 'mute'; const { participants } = await getGroupContext(context); const jids = targetJids(context); context.store.chats[context.chat].mutedUsers = [...new Set([...(context.store.chats[context.chat].mutedUsers || []), ...jids])]; await reply(context.sock, context.chat, context.message, jids.length ? `🔇 Silenciado: ${targetLabel(jids, participants)}` : 'Responde o etiqueta al usuario.') } },
  { name: 'unmute', async execute(context) { context.message._command = 'unmute'; const { participants } = await getGroupContext(context); const jids = targetJids(context); context.store.chats[context.chat].mutedUsers = (context.store.chats[context.chat].mutedUsers || []).filter((jid) => !jids.includes(jid)); await reply(context.sock, context.chat, context.message, jids.length ? `🔊 Habilitado: ${targetLabel(jids, participants)}` : 'Responde o etiqueta al usuario.') } },
