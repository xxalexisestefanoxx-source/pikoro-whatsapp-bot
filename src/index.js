...
async function start() {
  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect, qr }) => {
    ...
    if (!state.creds.registered && process.env.PAIRING_NUMBER && !pairingRequested) {
      ...
      setTimeout(async () => { try { const code = await sock.requestPairingCode(normalize(process.env.PAIRING_NUMBER)); console.log(`PAIRING_CODE=${code}`) } catch (error) { pairingRequested = false; console.error('No se pudo generar el código de vinculación.') } }, 3000)
    }
    if (connection === 'open') console.log(`${BOT_NAME} conectado.`)
    if (connection === 'close') { const code = lastDisconnect?.error?.output?.statusCode; if (code !== DisconnectReason.loggedOut) start(); else console.error('Sesión cerrada.') }
  })
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    await dispatch({ sock, m, jid, sender, args, command: raw, isGroup: !!metadata, groupJid: jid, isAdmin: admins.includes(sender), isOwner: isOwnerOf(sender), startedAt })
  })
}
const port = Number(process.env.PORT || 10000)
http.createServer((req, res) => { res.writeHead(200, { 'content-type': 'text/plain; charset=utf-8' }); res.end(`${BOT_NAME} activo\\n`) }).listen(port, '0.0.0.0', () => console.log(`Health server activo en ${port}`))
