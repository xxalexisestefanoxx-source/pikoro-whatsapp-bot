...
async function start() {
  ...
  sock.ev.on('connection.update', ({ connection, lastDisconnect, qr }) => {
    ...
    if (!state.creds.registered && process.env.PAIRING_NUMBER && !pairingRequested) {
      ...
      setTimeout(async () => {
        try { const code = await sock.requestPairingCode(normalize(process.env.PAIRING_NUMBER)); console.log(`PAIRING_CODE=${code}`) }
        catch (error) { pairingRequested = false; console.error('No se pudo generar el código de vinculación.') }
      }, 3000)
    }
